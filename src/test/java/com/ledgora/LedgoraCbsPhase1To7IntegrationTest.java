 ...
 *
 * All tests use @SpringBootTest with H2 in-memory database.
 * @Transactional ensures rollback after each test for isolation.
 */
@SpringBootTest
@ActiveProfiles("test")
...
class LedgoraCbsPhase1To7IntegrationTest {
    ...
    void testTenantCreation() {
        Tenant tenant = tenantService.createTenant("P1-TENANT-01", "Phase 1 Test Bank", LocalDate.now());
    ...
    void testCustomerCreationTenantScoped() {
        ...
        CustomerMaster customer = customerMasterRepository.save(CustomerMaster.builder()
                .tenant(tenant)
        ...
        assertTrue(customerMasterRepository.findByTenantIdAndCustomerNumber(
                tenant.getId(), "CUST-P1-001").isPresent());
    }
    ...
    void testAccountOpeningWithGlMapping() {
        ...
        assertEquals(data.tenant.getId(), data.account.getTenant().getId(), "Account must be tenant-scoped");
        assertEquals(data.branch.getId(), data.account.getBranch().getId(), "Account must be linked to branch");
    ...
    void testInitialFundingCreatesBalance() {
        ...
        assertEquals(0, new BigDecimal("10000.00").compareTo(balance.getActualTotalBalance()),
                "Initial funding should set actual balance");
        assertEquals(0, new BigDecimal("10000.00").compareTo(balance.getAvailableBalance()),
    ...
    void testCashDepositFullFlow() {
        ...
        List<Voucher> vouchers = voucherRepository.findByTenantIdAndBranchIdAndPostingDate(
        ...
        List<LedgerEntry> entries = ledgerEntryRepository.findByBusinessDateAndTenantId(
        ...
        AccountBalance balance = cbsBalanceEngine.getCbsBalance(data.account.getId());
        assertEquals(0, initialBalance.add(depositAmount).compareTo(balance.getActualTotalBalance()),
    ...
    void testCashWithdrawalFullFlow() {
        ...
        AccountBalance balance = cbsBalanceEngine.getCbsBalance(data.account.getId());
        assertEquals(0, initialBalance.subtract(withdrawAmount).compareTo(balance.getActualTotalBalance()),
    ...
    void testOnlineTransferDualLeg() {
        ...
        Account destAccount = createAccountInTenant(srcData.tenant, srcData.branch, "DST-P2-TRF", srcData.gl);
        ...
        assertEquals(0, new BigDecimal("8000.00").compareTo(srcBalance.getActualTotalBalance()),
        ...
        assertEquals(0, new BigDecimal("7000.00").compareTo(dstBalance.getActualTotalBalance()),
    ...
    void testWithdrawalExceedingBalanceFails() {
        ...
        assertThrows(RuntimeException.class, () -> transactionService.withdraw(dto),
    ...
    void testLedgerAppendOnly() {
        ...
        // Verify ledger entries are immutable - they have no update/delete operations in service layer
        // The LedgerEntry entity does not expose setter methods for key fields after creation
        // This is an architectural invariant: entries are created via VoucherService.postVoucher only
    ...
    void testIdempotencyDuplicateRejection() {
        ...
        TransactionDTO duplicate = TransactionDTO.builder()
                .transactionType("DEPOSIT")
                .destinationAccountNumber(data.account.getAccountNumber())
                .amount(new BigDecimal("1000.00"))
                .currency("INR")
        ...
        assertThrows(RuntimeException.class, () -> transactionService.deposit(duplicate),
    ...
    void testNegativeAmountInjection() {
        ...
        TransactionDTO negativeDto = TransactionDTO.builder()
                .transactionType("DEPOSIT")
                .destinationAccountNumber(data.account.getAccountNumber())
        ...
        assertThrows(RuntimeException.class, () -> transactionService.deposit(negativeDto),
                "Negative amount must be rejected by server-side validation");

        // Zero amount deposit
        TransactionDTO zeroDto = TransactionDTO.builder()
                .transactionType("DEPOSIT")
                .destinationAccountNumber(data.account.getAccountNumber())
        ...
        assertThrows(RuntimeException.class, () -> transactionService.deposit(zeroDto),
                "Zero amount must be rejected by server-side validation");

        // Negative amount withdrawal
        TransactionDTO negWdrDto = TransactionDTO.builder()
                .transactionType("WITHDRAWAL")
                .sourceAccountNumber(data.account.getAccountNumber())
        ...
        assertThrows(RuntimeException.class, () -> transactionService.withdraw(negWdrDto),
    ...
    void testBusinessDateOverrideBlocked() {
        ...
        assertThrows(BusinessDayClosedException.class, () -> transactionService.deposit(dto),
                "Transactions must be blocked when business day is not OPEN");
    }

    @Test @Order(23) @Transactional
    ...
    void testMakerCheckerEnforcement() {
        ...
        Voucher voucher = voucherService.createVoucher(
                ...
                VoucherDrCr.CR, new BigDecimal("1000.00"), new BigDecimal("1000.00"),
                "INR", LocalDate.now(), LocalDate.now(), "BATCH-MC", 1,
                data.maker, "Maker-checker test");
    ...
    void testCancelledVoucherCannotBeAuthorized() {
        ...
        Voucher voucher = voucherService.createVoucher(
                data.tenant, data.branch, data.account, data.gl,
                VoucherDrCr.CR, new BigDecimal("1000.00"), new BigDecimal("1000.00"),
                "INR", LocalDate.now(), LocalDate.now(), "BATCH-CANC", 1,
    ...
    void testDoubleAuthorizationBlocked() {
        ...
        Voucher voucher = voucherService.createVoucher(
                data.tenant, data.branch, data.account, data.gl,
                VoucherDrCr.CR, new BigDecimal("1000.00"), new BigDecimal("1000.00"),
                "INR", LocalDate.now(), LocalDate.now(), "BATCH-DAUTH", 1,
    ...
    void testUnauthorizedVoucherCannotBePosted() {
        ...
        Voucher voucher = voucherService.createVoucher(
                data.tenant, data.branch, data.account, data.gl,
                VoucherDrCr.CR, new BigDecimal("1000.00"), new BigDecimal("1000.00"),
                "INR", LocalDate.now(), LocalDate.now(), "BATCH-UA", 1,
                data.maker, "Unauth post test");
    ...
    void testFrozenAccountBlocksDebit() {
        ...
        // Apply debit freeze
        freezeControlRepository.save(CustomerFreezeControl.builder()
                .tenant(tenant)
    ...
    void testBatchTotalsAccumulate() {
        ...
        batchService.updateBatchTotals(batch.getId(), new BigDecimal("1000.00"), new BigDecimal("1000.00"));
        batchService.updateBatchTotals(batch.getId(), new BigDecimal("2000.00"), new BigDecimal("2000.00"));
        batchService.updateBatchTotals(batch.getId(), new BigDecimal("500.00"), new BigDecimal("500.00"));

        TransactionBatch updated = batchRepository.findById(batch.getId()).orElseThrow();
        assertEquals(0, new BigDecimal("3500.00").compareTo(updated.getTotalDebit()),
                "Batch debit totals must accumulate");
        assertEquals(0, new BigDecimal("3500.00").compareTo(updated.getTotalCredit()),
    ...
    void testCloseBalancedBatch() {
        ...
        TransactionBatch batch = batchService.getOrCreateOpenBatch(
                tenant.getId(), TransactionChannel.TELLER, LocalDate.now());
        batchService.updateBatchTotals(batch.getId(), new BigDecimal("5000.00"), new BigDecimal("5000.00"));

        // Close batch
        batchService.closeAllBatches(tenant.getId(), LocalDate.now());

        TransactionBatch closed = batchRepository.findById(batch.getId()).orElseThrow();
        assertEquals(BatchStatus.CLOSED, closed.getStatus(), "Balanced batch must close successfully");

        // Settle balanced batch
        assertDoesNotThrow(() -> batchService.settleAllBatches(tenant.getId(), LocalDate.now()),
                "Balanced batch must settle successfully");

        TransactionBatch settled = batchRepository.findById(batch.getId()).orElseThrow();
        assertEquals(BatchStatus.SETTLED, settled.getStatus(), "Batch must be SETTLED after settlement");
    }

    @Test @Order(34) @Transactional
    ...
    void testDifferentChannelsDifferentBatches() {
        ...
        assertNotEquals(tellerBatch.getId(), onlineBatch.getId(),
    ...
    void testGlDoubleEntryOnDeposit() {
        ...
        // Verify ledger entries balanced
        BigDecimal debits = ledgerEntryRepository.sumDebitsByBusinessDateAndTenantId(
                LocalDate.now(), data.tenant.getId());
        BigDecimal credits = ledgerEntryRepository.sumCreditsByBusinessDateAndTenantId(
                LocalDate.now(), data.tenant.getId());
        assertEquals(0, debits.compareTo(credits),
                "GL: Total debits must equal total credits");
    }
    ...
    void testTrialBalanceAfterTransactions() {
        ...
        // Deposit
        transactionService.deposit(TransactionDTO.builder()
                .transactionType("DEPOSIT")
                .destinationAccountNumber(data.account.getAccountNumber())
    ...
    void testNoOrphanLedgerEntries() {
        ...
        assertEquals(0, orphanCount, "There must be no orphan ledger entries (entries without transaction)");
    }

    @Test @Order(43) @Transactional
    ...
    void testShadowVsActualConsistent() {
        ...
        assertEquals(0, BigDecimal.ZERO.compareTo(balance.getShadowTotalBalance()),
                "Shadow balance should be zero after all vouchers are posted (no pending effects)");
    }

    @Test @Order(44) @Transactional
    ...
    void testLedgerEntriesPerTransactionBalanced() {
        ...
        Transaction txn = transactionService.deposit(TransactionDTO.builder()
                .transactionType("DEPOSIT")
                .destinationAccountNumber(data.account.getAccountNumber())
        ...
        assertEquals(0, txnDebits.compareTo(txnCredits),
    ...
    void testValidEodAllConditionsMet() {
        ...
        // Run EOD
        assertDoesNotThrow(() -> eodValidationService.runEod(tenant.getId()),
    ...
    void testEodBlockedByUnauthorizedVouchers() {
        ...
        voucherService.createVoucher(
                ...
                data.maker, "Unauthorized voucher for EOD test");
    ...
    void testEodBlockedByUnpostedVouchers() {
        ...
        Voucher voucher = voucherService.createVoucher(
                ...
                VoucherDrCr.CR, new BigDecimal("1000.00"), new BigDecimal("1000.00"),
                "INR", LocalDate.now(), LocalDate.now(), "BATCH-EOD", 1,
                data.maker, "Unposted voucher for EOD test");
        voucherService.authorizeVoucher(voucher.getId(), data.checker);
    ...
    void testEodRunThrowsOnFailure() {
        ...
        voucherService.createVoucher(
                data.tenant, data.branch, data.account, data.gl,
                VoucherDrCr.CR, new BigDecimal("1000.00"), new BigDecimal("1000.00"),
                "INR", LocalDate.now(), LocalDate.now(), "BATCH-FAIL", 1,
                data.maker, "Force EOD failure");
    ...
    void testCrossTenantAccountQueryBlocked() {
        ...
        var result = accountRepository.findByAccountNumberAndTenantId(
    ...
    void testCrossTenantTransactionBlocked() {
        ...
        SecurityContextHolder.getContext().setAuthentication(
                new UsernamePasswordAuthenticationToken(data2.maker.getUsername(), "N/A",
        ...
        assertThrows(RuntimeException.class, () -> transactionService.deposit(dto),
                "Cross-tenant transaction must be blocked");
    }

    @Test @Order(62) @Transactional
    ...
    void testTenantSpecificGlIsolation() {
        ...
        assertTrue(cbsGlBalanceService.isTenantGlBalanced(tenant1.getId()),
                "Tenant 1 GL must be balanced initially");
        assertTrue(cbsGlBalanceService.isTenantGlBalanced(tenant2.getId()),
                "Tenant 2 GL must be balanced initially");
    }

    @Test @Order(63) @Transactional
    ...
    void testVoucherIsolationBetweenTenants() {
        ...
        Voucher v1 = voucherService.createVoucher(
                ...
                VoucherDrCr.CR, new BigDecimal("1000.00"), new BigDecimal("1000.00"),
                "INR", LocalDate.now(), LocalDate.now(), "BATCH-V1", 1,
                data1.maker, "Tenant 1 voucher");

        // Switch context to tenant 2
        TenantContextHolder.setTenantId(data2.tenant.getId());
        Voucher v2 = voucherService.createVoucher(
                data2.tenant, data2.branch, data2.account, data2.gl,
                VoucherDrCr.CR, new BigDecimal("2000.00"), new BigDecimal("2000.00"),
                "INR", LocalDate.now(), LocalDate.now(), "BATCH-V2", 1,
                data2.maker, "Tenant 2 voucher");

        // Tenant 1 should not see tenant 2's voucher
        assertTrue(voucherRepository.findByIdAndTenantId(v2.getId(), data1.tenant.getId()).isEmpty(),
                "Tenant 1 must not see Tenant 2's voucher");

        // Tenant 2 should not see tenant 1's voucher
        assertTrue(voucherRepository.findByIdAndTenantId(v1.getId(), data2.tenant.getId()).isEmpty(),
                "Tenant 2 must not see Tenant 1's voucher");

        // Each tenant sees only its own voucher
        assertTrue(voucherRepository.findByIdAndTenantId(v1.getId(), data1.tenant.getId()).isPresent(),
                "Tenant 1 must see its own voucher");
        assertTrue(voucherRepository.findByIdAndTenantId(v2.getId(), data2.tenant.getId()).isPresent(),
                "Tenant 2 must see its own voucher");
    }

    @Test @Order(65) @Transactional
    ...
    void testBalanceIsolationBetweenTenants() {
        ...
        TenantContextHolder.setTenantId(data1.tenant.getId());
        SecurityContextHolder.getContext().setAuthentication(
                new UsernamePasswordAuthenticationToken(data1.maker.getUsername(), "N/A",
                        List.of(new SimpleGrantedAuthority("ROLE_TELLER"))));
        ...
        assertEquals(0, new BigDecimal("60000.00").compareTo(bal1.getActualTotalBalance()),
        ...
        assertEquals(0, new BigDecimal("10000.00").compareTo(bal2.getActualTotalBalance()),
                "Tenant 2 balance must be unaffected by Tenant 1's deposit");
    }

    @Test @Order(66) @Transactional
    ...
    void testCustomerDataIsolation() {
        ...
        assertNotEquals(t1Customers.get(0).getId(), t2Customers.get(0).getId(),
                "Same customer number in different tenants must be separate records");
    }

    @Test @Order(67) @Transactional
    @DisplayName("Phase 7.8: Tenant context validation - validateAccountForTransaction blocks mismatch")
    void testTenantContextValidationBlocksMismatch() {
        ...
        assertThrows(RuntimeException.class, () ->
                        customerValidationService.validateAccountForTransaction(
                                data1.account, data2.tenant.getId(), data1.branch.getId(), VoucherDrCr.DR),
    ...
    private Branch createBranch(String code) {
        ...
        return branchRepository.findByBranchCode(safeCode)
    ...
    private Account createAccountForCustomer(Tenant tenant, Branch branch, CustomerMaster cm, String accNo) {
    ...
    private Account createAccountInTenant(Tenant tenant, Branch branch, String accNo, GeneralLedger gl) {
    ...
    /**
     * Full test data setup for transaction-level tests.
     * Creates: Tenant, Branch, User (maker+checker), GL accounts, Customer, Account with balance,
     * Cash GL account (required for deposits/withdrawals), and sets security context.
     */
    private FullTestData setupFullTestData(String suffix) {
        ...
        GeneralLedger cashGl = glRepository.findByGlCode("1100")
                .orElseGet(() -> glRepository.save(GeneralLedger.builder()
                        .glCode("1100")
                        .glName("Cash GL")
        ...
        SecurityContextHolder.getContext().setAuthentication(
                new UsernamePasswordAuthenticationToken(maker.getUsername(), "N/A",
                        List.of(new SimpleGrantedAuthority("ROLE_TELLER"))));

        return new FullTestData(tenant, branch, account, gl, customer, maker, checker);
    }

    private record FullTestData(Tenant tenant, Branch branch, Account account,
                                GeneralLedger gl, CustomerMaster customer, User maker, User checker) {}
}
