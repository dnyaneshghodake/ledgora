...
class LedgoraTransactionVoucherInvariantIntegrationTest {
    ...
    void transferCreatesBalancedLedgerThroughVouchers() {
        ...
        assertEquals(0, debits.compareTo(credits), "Transaction journal must be balanced (debit == credit)");

        long voucherCountAfter = voucherRepository.count();
        assertEquals(voucherCountBefore + 2, voucherCountAfter,
        ...
        String expectedBatchCode = batch.getBatchCode() != null ? batch.getBatchCode() : "BATCH-" + batch.getId();

        List<Voucher> linkedVouchers = voucherRepository.findAll().stream()
                .filter(v -> expectedBatchCode.equals(v.getBatchCode()))
                .filter(v -> v.getPostingDate().equals(txn.getBusinessDate()))
                .filter(v -> v.getNarration() != null && v.getNarration().contains("Transfer"))
                .collect(Collectors.toList());

        assertTrue(linkedVouchers.size() >= 2, "Expected posted vouchers for transfer legs");
        linkedVouchers.forEach(v -> {
            assertEquals("Y", v.getAuthFlag(), "Voucher leg must be authorized before posting");
