...
/** CBS Voucher Lifecycle test suite. Tests: create, authorize, post, cancel voucher flow. */
@SpringBootTest
@ActiveProfiles("test")
...
class LedgoraVoucherLifecycleTest {
    ...
    void testCreateVoucherUpdatesShadow() {
        ...
        assertTrue(
                balance.getShadowTotalBalance().compareTo(BigDecimal.ZERO) != 0,
    ...
    void testPostVoucher() {
        ...
        Voucher voucher =
                voucherService.createVoucher(
                        ...
                        LocalDate.now(),
                        LocalDate.now(),
                        createOpenBatchCode(data.tenant, LocalDate.now()),
                        1,
                        data.maker,
                        "Test post voucher");
        ...
        AccountBalance balance = cbsBalanceEngine.getCbsBalance(data.account.getId());
        assertTrue(
                balance.getActualTotalBalance().compareTo(new BigDecimal("10000.0000")) > 0,
    ...
    void testCancelVoucher() {
        ...
        Voucher reversal =
                voucherService.cancelVoucher(voucher.getId(), data.maker, "Testing reversal");
    ...
    void testScrollNumberAutoIncrement() {
        ...
        assertTrue(v2.getScrollNo() > v1.getScrollNo(), "Scroll numbers should auto-increment");
    }
