...
/**
 * CBS Multi-Tenant Isolation test suite.
 * Tests: tenant isolation for vouchers, balances, customers, GL.
 */
@SpringBootTest
@ActiveProfiles("test")
...
class LedgoraMultiTenantIsolationTest {
    ...
    void testTenantBalanceIsolation() {
        ...
        cbsBalanceEngine.updateActualBalance(data1.account.getId(), new BigDecimal("10000.0000"), VoucherDrCr.CR);
        ...
        assertEquals(0, BigDecimal.ZERO.compareTo(bal2.getActualTotalBalance()),
    ...
    void testVoucherListTenantScoped() {
        ...
        voucherService.createVoucher(
                ...
                "INR", LocalDate.now(), LocalDate.now(), "BATCH-MT", 1,
                data2.maker, "T2 voucher");

        List<Voucher> t1Vouchers = voucherRepository.findByTenantIdAndBranchIdAndPostingDate(
                data1.tenant.getId(), data1.branch.getId(), LocalDate.now());
        List<Voucher> t2Vouchers = voucherRepository.findByTenantIdAndBranchIdAndPostingDate(
    ...
    void testCustomerMasterTenantScoped() {
        ...
        assertTrue(customerMasterRepository.findByTenantIdAndCustomerNumber(
                tenant1.getId(), "SHARED-CUST-001").isPresent());
        assertTrue(customerMasterRepository.findByTenantIdAndCustomerNumber(
                tenant2.getId(), "SHARED-CUST-001").isPresent());
