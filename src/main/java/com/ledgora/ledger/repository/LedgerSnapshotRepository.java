...
public interface LedgerSnapshotRepository extends JpaRepository<LedgerSnapshot, Long> {
    ...
    @Query(
            "SELECT ls FROM LedgerSnapshot ls WHERE ls.accountId = :accountId ORDER BY ls.snapshotDate DESC LIMIT 1")
    ...
    @Query(
            "SELECT ls FROM LedgerSnapshot ls WHERE ls.accountId = :accountId AND ls.snapshotDate <= :date ORDER BY ls.snapshotDate DESC LIMIT 1")
    Optional<LedgerSnapshot> findLatestByAccountIdAndDate(
