...
/**
 * PART 1 + PART 3: Ledger service - system of record for all financial entries. Creates immutable
 * ledger journals and entries. Publishes LedgerPostedEvent after successful posting. Now integrates
 * GL balance updates and multi-tenant support.
 */
@Service
public class LedgerService {
    ...
    /**
     * Create a ledger journal with entries for a transaction. Ensures SUM(debits) = SUM(credits)
     * before persisting. This is the core method that makes the ledger the system of record.
     */
    @Transactional
    public LedgerJournal postJournal(
            Transaction transaction,
            String description,
            LocalDate businessDate,
            List<JournalEntryRequest> entryRequests) {
        // Validate double-entry: SUM(debits) must equal SUM(credits)
        BigDecimal totalDebits =
                entryRequests.stream()
        ...
        if (totalDebits.compareTo(totalCredits) != 0) {
            throw new RuntimeException(
                    "Ledger integrity violation: SUM(debits)="
                            + totalDebits
                            + " != SUM(credits)="
                            + totalCredits);
        }
        ...
        if (entryRequests.size() < 2) {
            throw new RuntimeException(
                    "Ledger journal must have at least 2 entries for double-entry accounting");
        }
        ...
        for (JournalEntryRequest req : entryRequests) {
            ...
            if (glAccount != null) {
                ...
                BigDecimal credit =
                        req.entryType == EntryType.CREDIT ? req.amount : BigDecimal.ZERO;
        ...
        log.info(
                "Ledger journal posted: {} with {} entries for transaction {}",
                journal.getId(),
                entries.size(),
    ...
    public boolean validateLedgerIntegrity(LocalDate date) {
        ...
        if (!balanced) {
            log.error(
                    "Ledger integrity check FAILED for {}: debits={} credits={}",
                    date,
                    debits,
                    credits);
        }
    ...
    public static class JournalEntryRequest {
        ...
        public static JournalEntryRequest of(
                Account account,
                EntryType entryType,
                BigDecimal amount,
