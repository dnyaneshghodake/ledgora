...
/**
 * PART 5: Ledger snapshot service for performance optimization.
 * Creates periodic balance snapshots and computes balances from snapshots + delta entries.
 */
@Service
public class LedgerSnapshotService {
    ...
    public LedgerSnapshotService(LedgerSnapshotRepository snapshotRepository,
    ...
    public LedgerSnapshot createSnapshot(Long accountId) {
        ...
        Long lastEntryId = entries.isEmpty() ? 0L :
                entries.stream().mapToLong(LedgerEntry::getId).max().orElse(0L);

        LedgerSnapshot snapshot = LedgerSnapshot.builder()
                .accountId(accountId)
        ...
        log.info("Snapshot created for account {}: balance={}, lastEntryId={}",
    ...
    /**
     * Calculate balance using snapshot + delta entries (entries after snapshot).
     * This is the optimized balance calculation for PART 5.
     */
    public BigDecimal calculateBalanceFromSnapshot(Long accountId) {
        Optional<LedgerSnapshot> latestSnapshot = snapshotRepository.findLatestByAccountId(accountId);

        if (latestSnapshot.isPresent()) {
            LedgerSnapshot snapshot = latestSnapshot.get();
            // Get entries after the snapshot
            BigDecimal deltaCredits = entryRepository.sumCreditsAfterEntryId(accountId, snapshot.getLastEntryId());
            BigDecimal deltaDebits = entryRepository.sumDebitsAfterEntryId(accountId, snapshot.getLastEntryId());
