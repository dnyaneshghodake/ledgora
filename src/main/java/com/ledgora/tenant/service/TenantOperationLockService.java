...
/**
 * CBS-grade Tenant Operation Lock Service. Ensures mutual exclusion of critical
 * batch operations (EOD, Settlement, Reconciliation) per tenant.
 *
 * <p>Usage pattern:
 * <pre>
 ...
 * </pre>
 *
 * <p>The lock is implemented via SELECT ... FOR UPDATE on the lock row.
 * If another process is already running EOD or Settlement for the same tenant,
 * the acquire call blocks until the first process commits/rollbacks.
 *
 * <p>Additionally, the lock row tracks which operation is active (for dashboard
 * visibility and stale lock detection).
 */
@Service
public class TenantOperationLockService {
    ...
    public TenantOperationLock acquireLock(Long tenantId, String operation, String username) {
        ...
        TenantOperationLock lock = lockRepository.findByTenantId(tenantId)
                .orElseGet(() -> {
                    TenantOperationLock newLock = TenantOperationLock.builder()
                            .tenantId(tenantId)
                            .build();
        ...
        lock = lockRepository.findByTenantIdWithLock(tenantId)
                .orElseThrow(() -> new RuntimeException(
                        "Failed to acquire operation lock for tenant " + tenantId));

        // Check if another operation is already active (stale lock detection)
        if (lock.getLockedByOperation() != null && lock.getLockedAt() != null
            ...
            if (lock.getLockedAt().isAfter(staleCutoff)) {
                throw new RuntimeException(
                        "Tenant " + tenantId + " is already locked by operation '"
                                + lock.getLockedByOperation()
                                + "' (acquired by " + lock.getLockedByUser()
                                + " at " + lock.getLockedAt()
                                + "). Cannot run " + operation + " concurrently.");
            } else {
                log.warn("Stale lock detected for tenant {} operation {} (locked at {}). Overriding.",
                        tenantId, lock.getLockedByOperation(), lock.getLockedAt());
            }
        }
        ...
        log.info("Operation lock acquired: tenant={} operation={} user={}",
                tenantId, operation, username);
        return lock;
    }
    ...
    public void releaseLock(Long tenantId) {
        lockRepository.findByTenantId(tenantId).ifPresent(lock -> {
            ...
            lockRepository.save(lock);
            log.info("Operation lock released: tenant={} operation={}", tenantId, operation);
        });
    }
    ...
    public boolean isLocked(Long tenantId) {
        return lockRepository.findByTenantId(tenantId)
    ...
    public String getLockStatus(Long tenantId) {
        return lockRepository.findByTenantId(tenantId)
                ...
                .map(lock -> lock.getLockedByOperation() + " by " + lock.getLockedByUser()
                        + " since " + lock.getLockedAt())
                .orElse("UNLOCKED");
    }
}
