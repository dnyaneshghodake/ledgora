...
/**
 * CBS-grade account validation service. Before any transaction, validates: - Customer is ACTIVE -
 * No debit freeze (for debit operations) - No credit freeze (for credit operations) - Tenant match
 * - Branch match Blocks transaction if validation fails.
 */
@Service
public class CbsCustomerValidationService {
    ...
    public CbsCustomerValidationService(
    ...
    public void validateAccountForTransaction(
            Account account, Long tenantId, Long branchId, VoucherDrCr drCr) {
        // Tenant match validation
        if (account.getTenant() == null || !account.getTenant().getId().equals(tenantId)) {
            throw new RuntimeException(
                    "Tenant mismatch: account tenant does not match transaction tenant");
        }
        ...
        if (branchId != null
            ...
            log.warn(
                    "Branch mismatch for account {}: account branch {} != transaction branch {}",
    ...
    private void validateFreezeControl(CustomerMaster customer, Long tenantId, VoucherDrCr drCr) {
        Optional<CustomerFreezeControl> freezeOpt =
                freezeControlRepository.findByCustomerMasterIdAndTenantId(
        ...
        if (freezeOpt.isPresent()) {
            ...
            if (drCr == VoucherDrCr.DR && Boolean.TRUE.equals(freeze.getDebitFreeze())) {
                throw new RuntimeException(
                        "Debit freeze active on customer "
                                + customer.getCustomerNumber()
                                + ". Reason: "
                                + freeze.getDebitFreezeReason());
            }
            if (drCr == VoucherDrCr.CR && Boolean.TRUE.equals(freeze.getCreditFreeze())) {
                throw new RuntimeException(
                        "Credit freeze active on customer "
                                + customer.getCustomerNumber()
                                + ". Reason: "
                                + freeze.getCreditFreezeReason());
            }
        }
    }
}
