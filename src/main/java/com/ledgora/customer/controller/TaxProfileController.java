...
public class TaxProfileController {
    ...
    public TaxProfileController(CustomerRepository customerRepository,
    ...
    public String showForm(@RequestParam("customerId") Long customerId, Model model) {
        ...
        Optional<CustomerMaster> cmOpt = customerMasterRepository
                .findByTenantIdAndNationalId(tenantId, customer.getNationalId());
        if (cmOpt.isPresent()) {
            Optional<CustomerTaxProfile> existing = taxProfileRepository
                    .findByCustomerMasterIdAndTenantId(cmOpt.get().getId(), tenantId);
    ...
    @PostMapping("/save")
    public String saveTaxProfile(@RequestParam("customerId") Long customerId,
                                 ...
                                 @RequestParam(value = "taxResidencyStatus", defaultValue = "RESIDENT") String taxResidencyStatus,
        ...
        try {
            ...
            CustomerMaster cm = customerMasterRepository
                    .findByTenantIdAndNationalId(tenantId, customer.getNationalId())
                    .orElseThrow(() -> new RuntimeException(
                            "No CBS Customer Master found for national ID: " + customer.getNationalId()
                            + ". Please create the customer in CBS Customer Master first."));

            // Find or create tax profile
            CustomerTaxProfile tp = taxProfileRepository
                    .findByCustomerMasterIdAndTenantId(cm.getId(), tenantId)
                    .orElseGet(() -> CustomerTaxProfile.builder()
                            .tenant(tenant)
                            .customerMaster(cm)
                            .build());
