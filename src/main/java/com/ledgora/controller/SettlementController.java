...
import com.ledgora.service.SettlementService;
...
public class SettlementController {
    ...
    @GetMapping
    public String listSettlements(@RequestParam(value = "status", required = false) String status,
                                  Model model) {
        if (status != null && !status.isEmpty()) {
            model.addAttribute("settlements", settlementService.getSettlementsByStatus(SettlementStatus.valueOf(status)));
    ...
    @PostMapping("/process")
    public String processSettlement(@RequestParam("settlementDate")
                                    @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate date,
                                    RedirectAttributes redirectAttributes) {
        try {
            Settlement settlement = settlementService.processSettlement(date);
            redirectAttributes.addFlashAttribute("message",
                    "Settlement processed successfully! Ref: " + settlement.getSettlementRef()
                    + " | Transactions: " + settlement.getTransactionCount());
