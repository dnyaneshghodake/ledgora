...
/**
 * Controller for the Voucher lifecycle UI screens. Routes: /vouchers, /vouchers/create,
 * /vouchers/pending, /vouchers/posted, /vouchers/cancelled
 *
 * <p>Voucher lifecycle: Create (Maker) -> Authorize (Checker) -> Post -> (Cancel via reversal)
 * Vouchers created here remain in PENDING authorization state (authFlag=N) until a checker
 * authorizes them via the pending vouchers screen.
 */
@Controller
...
public class VoucherController {
    ...
    @GetMapping
    @PreAuthorize(
            "hasAnyRole('MAKER', 'CHECKER', 'AUDITOR', 'ADMIN', 'MANAGER', 'TELLER', 'OPERATIONS')")
    ...
    /**
     * Voucher detail view: shows header, linked transaction, ledger entries, batch reference,
     * maker/checker, and derived status timeline.
     *
     * <p>Accessible by MAKER (own vouchers), CHECKER, AUDITOR (read-only), ADMIN, MANAGER.
     */
    @GetMapping("/{id}")
    @PreAuthorize(
            "hasAnyRole('MAKER', 'CHECKER', 'AUDITOR', 'ADMIN', 'MANAGER', 'TELLER', 'OPERATIONS')")
    ...
    public String viewVoucher(@PathVariable Long id, Model model) {
        ...
        if (voucher.getTransaction() != null) {
            List<Voucher> relatedVouchers =
                    voucherRepository.findByTransactionId(voucher.getTransaction().getId());
        ...
        String checkerUsername =
                voucher.getChecker() != null ? voucher.getChecker().getUsername() : "";
    ...
    /**
     * PART 4: Handle voucher creation form POST. Creates a pair of vouchers (DR + CR) in PENDING
     * authorization state. Vouchers must be authorized by a checker before they can be posted.
     */
    @PostMapping("/create")
    @PreAuthorize("hasAnyRole('MAKER', 'ADMIN', 'MANAGER', 'TELLER')")
    public String createVoucher(
        ...
        try {
            ...
            if (debitAccountNumber == null
                    || debitAccountNumber.isBlank()
                    || creditAccountNumber == null
                    || creditAccountNumber.isBlank()) {
            ...
            Account debitAccount =
                    accountRepository
                            .findByAccountNumberAndTenantId(debitAccountNumber, tenantId)
                            .orElseThrow(
                                    () ->
            ...
            Account creditAccount =
                    accountRepository
                            .findByAccountNumberAndTenantId(creditAccountNumber, tenantId)
                            .orElseThrow(
                                    () ->
                                            new RuntimeException(
            ...
            // Create DR+CR voucher pair atomically (single transaction — if CR fails, DR rolls
            // back)
            ...
            redirectAttributes.addFlashAttribute(
                    "message",
                    "Voucher pair created (PENDING authorization). DR scroll #"
                            + drVoucher.getScrollNo()
                            + ", CR scroll #"
                            + crVoucher.getScrollNo()
                            + " for "
                            + currency
                            + " "
                            + amount
    ...
    /**
     * Authorize a pending voucher (checker action). Checker must not be the same as maker (enforced
     * by VoucherService).
     */
    ...
    /**
     * Post an authorized voucher (creates immutable ledger entries). Typically called after
     * authorization to complete the voucher lifecycle.
     */
    @PostMapping("/{id}/post")
    @PreAuthorize("hasAnyRole('CHECKER', 'ADMIN', 'MANAGER')")
    public String postVoucher(@PathVariable Long id, RedirectAttributes redirectAttributes) {
        try {
            // Day status + tenant isolation + DR==CR all enforced inside VoucherService.postVoucher
            Voucher posted = voucherService.postVoucher(id);
            redirectAttributes.addFlashAttribute(
                    "message", "Voucher " + posted.getVoucherNumber() + " posted to ledger.");
    ...
    /**
     * Reject (cancel) a pending voucher (checker action). Creates a reversal voucher per CBS cancel
     * pattern.
     */
