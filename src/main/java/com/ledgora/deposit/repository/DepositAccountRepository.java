...
public interface DepositAccountRepository extends JpaRepository<DepositAccount, Long> {
    ...
    @Query(
            "SELECT da FROM DepositAccount da WHERE da.tenant.id = :tenantId AND da.status = 'ACTIVE'")
    List<DepositAccount> findActiveByTenantId(@Param("tenantId") Long tenantId);

    @Query(
            "SELECT da FROM DepositAccount da WHERE da.tenant.id = :tenantId "
    ...
    @Query(
            "SELECT COALESCE(SUM(da.principalAmount), 0) FROM DepositAccount da "
    ...
    @Query(
            "SELECT COALESCE(SUM(da.interestAccrued), 0) FROM DepositAccount da "
                    + "WHERE da.tenant.id = :tenantId AND da.status = 'ACTIVE'")
