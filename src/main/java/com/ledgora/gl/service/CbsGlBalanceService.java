 ...
 * 
 * On voucher post:
 *   If DR: Debit account, Credit GL
 *   If CR: Credit account, Debit GL
 * 
 * Updates:
 *   - gl_actual_balance (on GlBranchBalance)
 *   - branch_gl_balance (aggregate per branch)
 *   - tenant_gl_balance (aggregate per tenant)
 * 
 * GL must be tenant + branch scoped.
 */
@Service
public class CbsGlBalanceService {
    ...
    public CbsGlBalanceService(GlBranchBalanceRepository branchBalanceRepository,
    ...
    /**
     * Update GL balance at branch and tenant level.
     * Called on voucher posting.
     */
    @Transactional
    public void updateGlBalances(Long tenantId, Long branchId, GeneralLedger gl, BigDecimal delta) {
        ...
        log.debug("CBS GL balances updated: tenant={}, branch={}, gl={}, delta={}",
                tenantId, branchId, gl.getGlCode(), delta);
    }
     ...
     */
    @Transactional
    public void updateBranchGlBalance(Long tenantId, Long branchId, GeneralLedger gl, BigDecimal delta) {
        GlBranchBalance branchBalance = branchBalanceRepository
    ...
    public void updateTenantGlBalance(Long tenantId, GeneralLedger gl, BigDecimal delta) {
        GlTenantBalance tenantBalance = tenantBalanceRepository
    ...
    public boolean isBranchGlBalanced(Long tenantId, Long branchId) {
        BigDecimal sum = branchBalanceRepository.sumBalanceByTenantIdAndBranchId(tenantId, branchId);
