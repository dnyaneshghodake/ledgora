...
public interface GlTenantBalanceRepository extends JpaRepository<GlTenantBalance, Long> {
    ...
    @Query(
            "SELECT gtb FROM GlTenantBalance gtb WHERE gtb.tenant.id = :tenantId AND gtb.gl.id = :glId")
    Optional<GlTenantBalance> findByTenantIdAndGlIdWithLock(
            @Param("tenantId") Long tenantId, @Param("glId") Long glId);
    ...
    @Query(
            "SELECT COALESCE(SUM(gtb.glActualBalance), 0) FROM GlTenantBalance gtb WHERE gtb.tenant.id = :tenantId")
