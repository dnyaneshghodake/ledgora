...
/**
 * Service for managing account liens.
 * Lien reduces available balance. Creation/release require maker-checker.
 * Expired liens auto-release via scheduler.
 */
@Service
public class AccountLienService {
    ...
    public AccountLienService(AccountLienRepository lienRepository,
     ...
     */
    @Transactional
    public AccountLien createLien(Long tenantId, Long accountId, BigDecimal lienAmount,
                                   LienType lienType, LocalDate startDate, LocalDate endDate,
                                   String lienReference, String remarks) {
        ...
        AccountBalance balance = accountBalanceRepository.findByAccountId(accountId)
                .orElseThrow(() -> new RuntimeException("Account balance not found for account: " + accountId));
        ...
        if (currentLiens.add(lienAmount).compareTo(balance.getLedgerBalance()) > 0) {
            throw new RuntimeException("Lien amount would exceed ledger balance. Ledger: "
                    + balance.getLedgerBalance() + ", Existing liens: " + currentLiens + ", New lien: " + lienAmount);
        }
        ...
        approvalService.submitForApproval("ACCOUNT_LIEN", saved.getId(),
                "Lien: account=" + account.getAccountNumber() + " amount=" + lienAmount + " type=" + lienType);

        Long userId = currentUser != null ? currentUser.getId() : null;
        auditService.logEvent(userId, "LIEN_CREATE", "ACCOUNT_LIEN", saved.getId(),
                "Lien created for account " + account.getAccountNumber() + " amount=" + lienAmount, null);

        log.info("Lien created (PENDING): account={}, amount={}, type={}", account.getAccountNumber(), lienAmount, lienType);
        return saved;
    }
    ...
    public AccountLien approveLien(Long lienId) {
        AccountLien lien = lienRepository.findById(lienId)
        ...
        if (lien.getCreatedBy() != null && currentUser != null
            ...
            throw new RuntimeException("Cannot approve your own lien request (maker-checker violation)");
        }
        ...
        Long userId = currentUser != null ? currentUser.getId() : null;
        auditService.logEvent(userId, "LIEN_APPROVE", "ACCOUNT_LIEN", saved.getId(),
                "Lien approved: amount=" + lien.getLienAmount(), null);
