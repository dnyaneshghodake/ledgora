 ...
 *
 * RBI Requirement: Branch books must independently balance.
 *
 * Accounting model:
 *   Branch A (source):  DR Customer A  →  CR IBC_OUT_A     (Branch A balanced)
 *   Branch B (dest):    DR IBC_IN_B    →  CR Customer B     (Branch B balanced)
 *   Settlement:         DR IBC_IN      →  CR IBC_OUT        (Clearing zeroed)
 *
 * IBC_OUT accounts use account number pattern: IBC-OUT-<branchCode>
 * IBC_IN accounts use account number pattern:  IBC-IN-<branchCode>
 */
@Service
public class InterBranchClearingService {
    ...
    public InterBranchClearingService(InterBranchTransferRepository ibcRepository,
     ...
     */
    @Transactional
    public InterBranchTransfer createTransfer(Tenant tenant, Branch fromBranch, Branch toBranch,
                                               BigDecimal amount, String currency,
                                               com.ledgora.transaction.entity.Transaction referenceTransaction,
                                               LocalDate businessDate, User createdBy, String narration) {
        ...
        log.info("InterBranchTransfer created: id={} from={} to={} amount={} date={}",
                transfer.getId(), fromBranch.getBranchCode(), toBranch.getBranchCode(),
                amount, businessDate);

        auditService.logEvent(createdBy != null ? createdBy.getId() : null,
                "IBC_TRANSFER_CREATED", "INTER_BRANCH_TRANSFER", transfer.getId(),
                "IBC transfer " + transfer.getId() + ": " + fromBranch.getBranchCode()
                + " → " + toBranch.getBranchCode() + " amount=" + amount, null);
    ...
    public void markSent(Long transferId) {
        InterBranchTransfer transfer = ibcRepository.findById(transferId)
    ...
    public void markReceived(Long transferId) {
        ...
        if (transfer.getStatus() != InterBranchTransferStatus.SENT) {
            throw new RuntimeException("IBC transfer " + transferId
                    + " cannot be marked RECEIVED — current status: " + transfer.getStatus());
        }
    ...
    /**
     * Resolve the IBC_OUT clearing account for a branch.
     * Account number pattern: IBC-OUT-<branchCode>
     */
    public Account resolveIbcOutAccount(Long tenantId, Branch branch) {
        String accountNumber = "IBC-OUT-" + branch.getBranchCode();
        return accountRepository.findByAccountNumberAndTenantId(accountNumber, tenantId)
                .orElseThrow(() -> new com.ledgora.common.exception.GovernanceException(
                        "IBC_ACCOUNT_MISSING",
                        "Inter-branch clearing account " + accountNumber + " not found for tenant "
                        + tenantId + ". Seed IBC accounts in DataInitializer."));
    }

    /**
     * Resolve the IBC_IN clearing account for a branch.
     * Account number pattern: IBC-IN-<branchCode>
     */
    ...
    /**
     * Settle all RECEIVED transfers for a business date.
     * Called during settlement processing to mark transfers as SETTLED.
     *
     * @return number of transfers settled
     */
    @Transactional
    public int settleTransfers(Long tenantId, LocalDate businessDate) {
        List<InterBranchTransfer> received = ibcRepository.findByTenantIdAndBusinessDateAndStatus(
        ...
        for (InterBranchTransfer transfer : received) {
            ...
            auditService.logEvent(null, "IBC_TRANSFER_SETTLED", "INTER_BRANCH_TRANSFER", transfer.getId(),
                    "IBC transfer " + transfer.getId() + " settled: "
                    + transfer.getFromBranch().getBranchCode() + " → " + transfer.getToBranch().getBranchCode()
                    + " amount=" + transfer.getAmount(), null);
        }

        if (count > 0) {
            log.info("Settled {} inter-branch transfer(s) for tenant {} date {}", count, tenantId, businessDate);
        }
        return count;
    }

    /**
     * Validate clearing balance for EOD.
     * Returns error message if imbalanced, null if OK.
     */
    public String validateClearingBalance(Long tenantId, LocalDate businessDate) {
        ...
        BigDecimal receivedTotal = ibcRepository.sumReceivedAmountByTenantAndDate(tenantId, businessDate);

        long unsettled = ibcRepository.countUnsettledByTenantAndDate(tenantId, businessDate);
        if (unsettled > 0) {
            return "EOD blocked: " + unsettled + " unsettled inter-branch transfer(s) for " + businessDate
                    + ". SENT total=" + sentTotal + ", RECEIVED total=" + receivedTotal
