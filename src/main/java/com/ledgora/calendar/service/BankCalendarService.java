...
/**
 * Banking Calendar Service for RBI-grade calendar discipline. Enforces WORKING_DAY vs HOLIDAY
 * gating for transactions. Calendar changes require maker-checker approval.
 */
@Service
public class BankCalendarService {
    ...
    public BankCalendarService(
            BankCalendarRepository calendarRepository,
    ...
    public boolean isHoliday(Long tenantId, LocalDate date) {
        Optional<BankCalendar> calendar =
                calendarRepository.findApprovedByTenantIdAndDate(tenantId, date);
        return calendar.isPresent() && calendar.get().isHoliday();
    }

    /**
     * Check if a date is a working day for the given tenant. If no calendar entry exists, defaults
     * to working day (weekdays) or holiday (weekends).
     */
    ...
    /**
     * Validate that a transaction is allowed on the given date. Manual (TELLER) transactions are
     * blocked on holidays. ATM/System transactions are allowed only if configured.
     */
    public void validateTransactionAllowed(
            Long tenantId, LocalDate date, TransactionChannel channel) {
        if (isWorkingDay(tenantId, date)) {
            return; // Working day - all transactions allowed
        }

        // It's a holiday
        Optional<BankCalendar> calendarOpt =
                calendarRepository.findApprovedByTenantIdAndDate(tenantId, date);
        BankCalendar calendar = calendarOpt.orElse(null);

        if (channel == TransactionChannel.TELLER) {
            throw new RuntimeException(
                    "Manual transactions are blocked on holidays. Date: " + date);
        }

        if (channel == TransactionChannel.ATM) {
            if (calendar == null || !Boolean.TRUE.equals(calendar.getAtmAllowed())) {
                throw new RuntimeException(
                        "ATM transactions are not allowed on this holiday. Date: " + date);
            }
        }
        ...
        if (channel == TransactionChannel.BATCH) {
            if (calendar == null || !Boolean.TRUE.equals(calendar.getSystemTransactionsAllowed())) {
                throw new RuntimeException(
                        "System/batch transactions are not allowed on this holiday. Date: " + date);
            }
        }
            ...
            if (calendar == null || !Boolean.TRUE.equals(calendar.getAtmAllowed())) {
                throw new RuntimeException(
                        "Digital banking transactions ("
                                + channel
                                + ") are not allowed on this holiday. Date: "
                                + date);
            }
        }
    }
    ...
    @Transactional
    public BankCalendar createCalendarEntry(
            Long tenantId,
            LocalDate date,
            String dayType,
            String holidayName,
        ...
        if (date.isBefore(currentBusinessDate)) {
            throw new RuntimeException(
                    "Cannot create calendar entry for past date: "
                            + date
        ...
        approvalService.submitForApproval(
                "BANK_CALENDAR", saved.getId(), "Calendar entry: " + date + " -> " + dayType);
        ...
        auditService.logEvent(
                ...
                "BANK_CALENDAR",
                saved.getId(),
                "Calendar entry created: " + date + " type=" + dayType,
                null);

        log.info(
                "Calendar entry created (PENDING): date={}, type={}, tenant={}",
                date,
                dayType,
                tenantId);
        return saved;
    ...
    public BankCalendar approveCalendarEntry(Long calendarId) {
                        ...
                        .orElseThrow(
                                () ->
                                        new RuntimeException(
                                                "Calendar entry not found: " + calendarId));
        ...
        if (calendar.getCreatedBy() != null
            ...
            throw new RuntimeException(
                    "Cannot approve your own calendar entry (maker-checker violation)");
        }
        ...
        auditService.logEvent(
                ...
                "BANK_CALENDAR",
                saved.getId(),
                "Calendar entry approved: " + calendar.getCalendarDate(),
                null);

        log.info(
                "Calendar entry approved: date={}, type={}",
    ...
    public BankCalendar rejectCalendarEntry(Long calendarId, String reason) {
        ...
        auditService.logEvent(
                ...
                "BANK_CALENDAR",
                saved.getId(),
                "Calendar entry rejected: " + reason,
                null);
    ...
    public List<BankCalendar> getPendingCalendarEntries(Long tenantId) {
        return calendarRepository.findByTenantIdAndApprovalStatus(
