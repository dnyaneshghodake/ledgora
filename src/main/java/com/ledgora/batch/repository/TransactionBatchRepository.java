...
public interface TransactionBatchRepository extends JpaRepository<TransactionBatch, Long> {
    ...
    @Query("SELECT tb FROM TransactionBatch tb WHERE tb.tenant.id = :tenantId AND tb.batchType = :batchType AND tb.businessDate = :businessDate AND tb.status = :status")
    ...
    @Query("SELECT tb FROM TransactionBatch tb WHERE tb.tenant.id = :tenantId AND tb.businessDate = :businessDate")
    List<TransactionBatch> findByTenantIdAndBusinessDate(@Param("tenantId") Long tenantId, @Param("businessDate") LocalDate businessDate);

    @Query("SELECT tb FROM TransactionBatch tb WHERE tb.tenant.id = :tenantId AND tb.status = :status")
    List<TransactionBatch> findByTenantIdAndStatus(@Param("tenantId") Long tenantId, @Param("status") BatchStatus status);
