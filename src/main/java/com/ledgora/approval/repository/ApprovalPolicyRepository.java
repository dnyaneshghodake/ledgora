...
public interface ApprovalPolicyRepository extends JpaRepository<ApprovalPolicy, Long> {
    ...
    /**
     * Find active policies matching tenant + transaction type + specific channel. Ordered by most
     * specific first (exact channel match before wildcard).
     */
    @Query(
            "SELECT p FROM ApprovalPolicy p WHERE p.tenant.id = :tenantId "
