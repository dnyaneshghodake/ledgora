 ...
 *
 * Determines whether a transaction should be auto-authorized or requires maker-checker approval.
 *
 * Decision hierarchy:
 *   1. Governance overrides (always require approval): reversals, backdated entries
 *   2. System-only transactions (always auto-authorize): interest accrual, charges, EOD via BATCH channel
 *   3. Policy table lookup: match by tenant + type + channel + amount range
 *   4. Default fallback: if no policy found, require approval (fail-safe)
 *
 * The policy table is configurable per tenant, allowing each bank/branch
 * to set their own teller limits and approval thresholds.
 */
@Service
public class ApprovalPolicyService {
     ...
     */
    public boolean isAutoAuthorized(Long tenantId, TransactionType transactionType,
                                     TransactionChannel channel, BigDecimal amount,
                                     boolean isReversal, boolean isBackdated) {
        ...
        List<ApprovalPolicy> policies = policyRepository.findMatchingPolicies(
                tenantId, typeStr, channelStr);

        if (policies.isEmpty()) {
            // Default fallback: no policy configured = require approval (fail-safe)
            log.info("No approval policy found for tenant={} type={} channel={}. Defaulting to REQUIRE_APPROVAL.",
                    tenantId, typeStr, channelStr);
            return false;
        }
        ...
        for (ApprovalPolicy policy : policies) {
            if (amountInRange(amount, policy.getMinAmount(), policy.getMaxAmount())) {
                ...
                log.info("Approval policy matched: id={} desc='{}' autoAuthorize={} for type={} channel={} amount={}",
                        policy.getId(), policy.getDescription(), autoAuth, typeStr, channelStr, amount);
                return autoAuth;
            }
        }

        // Amount exceeds all configured ranges -> require approval
        log.info("Amount {} exceeds all policy ranges for type={} channel={}. Requiring approval.",
                amount, typeStr, channelStr);
        return false;
    }
