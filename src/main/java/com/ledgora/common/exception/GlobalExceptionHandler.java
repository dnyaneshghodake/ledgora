...
/**
 * Global exception handler for CBS application. Routes all exceptions to a centralized error page
...
public class GlobalExceptionHandler {
    ...
    /**
     * PART 5: Handle static resource 404s (e.g., .map files) without ERROR logging. Browser
     * requests for source maps should not pollute application logs.
     */
    @ExceptionHandler(NoResourceFoundException.class)
    @ResponseStatus(HttpStatus.NOT_FOUND)
    public String handleStaticResourceNotFound(
        ...
        populateErrorModel(
                model, request, "Page Not Found", "The requested resource was not found.", "404");
        return "error/error";
    }
    ...
    @ResponseStatus(HttpStatus.NOT_FOUND)
    public String handleTransactionNotFound(
        ...
        if (response.isCommitted()) {
            log.debug(
                    "Response already committed, cannot forward for TransactionNotFoundException");
            return null;
        }
        populateErrorModel(
                model, request, "Transaction Not Found", ex.getMessage(), "TXN_NOT_FOUND");
    ...
    public String handleBusinessException(
            BusinessException ex, Model model, HttpServletRequest request) {
        populateErrorModel(
                model, request, "Business Rule Violation", ex.getMessage(), ex.getErrorCode());
    ...
    @ResponseStatus(HttpStatus.BAD_REQUEST)
    public String handleInvalidAmount(
            InvalidTransactionAmountException ex, Model model, HttpServletRequest request) {
        populateErrorModel(
                model, request, "Invalid Transaction Amount", ex.getMessage(), "INVALID_AMOUNT");
    ...
    @ResponseStatus(HttpStatus.BAD_REQUEST)
    public String handleScriptInjection(
            ScriptInjectionException ex, Model model, HttpServletRequest request) {
        populateErrorModel(
                model,
                request,
                "Invalid Input",
                ...
                "SCRIPT_INJECTION");
        log.warn(
                "Script injection attempt detected: {} - URI: {}",
                ex.getMessage(),
    ...
    public String handleIllegalArgument(
        ...
        populateErrorModel(model, request, "Validation Error", ex.getMessage(), "VALIDATION_ERROR");
    ...
    public String handleInsufficientBalance(
            InsufficientBalanceException ex, Model model, HttpServletRequest request) {
        populateErrorModel(
                model, request, "Insufficient Balance", ex.getMessage(), "INSUFFICIENT_BALANCE");
    ...
    public String handleEodClosed(
            BusinessDayClosedException ex, Model model, HttpServletRequest request) {
        populateErrorModel(
                model,
                request,
                "Business Day Closed",
    ...
    @ResponseStatus(HttpStatus.FORBIDDEN)
    public String handleTenantIsolation(
            TenantIsolationException ex, Model model, HttpServletRequest request) {
        populateErrorModel(
                model,
                request,
    ...
    /**
     * PART 10: Generic error handler with response.isCommitted() guard to prevent "Cannot forward
     * to error page after response has been committed".
     */
    @ExceptionHandler(Throwable.class)
    @ResponseStatus(HttpStatus.INTERNAL_SERVER_ERROR)
    public String handleGenericError(
            Throwable ex, Model model, HttpServletRequest request, HttpServletResponse response) {
        if (response.isCommitted()) {
            log.error(
                    "Response already committed at {}: {}",
                    request.getRequestURI(),
                    ex.getMessage());
            return null;
        }
    ...
    private void populateErrorModel(
            Model model,
            HttpServletRequest request,
            String title,
            String message,
